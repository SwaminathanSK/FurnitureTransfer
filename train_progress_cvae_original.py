"""
Training script for Progress-Supervised Conditional VAE using original expert demo datasets
...
Uses the original expert demonstration datasets with low randomness.
"""
...
class OriginalExpertDataset(Dataset):
    """
    Dataset for original expert demonstrations with progress labels.
    Uses the original zarr structure with robot_state + parts_poses.
    """
    ...
    def __init__(
        ...
        print(f"Loading original expert dataset from {dataset_path}")
        dataset = zarr.open(dataset_path, mode='r')

        # Load original format with separate robot_state and parts_poses
        ...
        # Create progress labels from episode structure
        print("Creating progress labels from episode structure...")
...
class ProgressCVAETrainer:
    ...
    def train_step(
        ...
        if torch.isnan(total_loss) or torch.isinf(total_loss):
            print(f"WARNING: Invalid total_loss detected: {total_loss.item()}")
            for k, v in loss_dict.items():
                if isinstance(v, torch.Tensor):
                    print(f"  {k}: {v.item()}")
            result = {k: float('inf') if k == 'total_loss' else 0.0 for k in loss_dict.keys()}
        ...
        if grad_norm > 100.0:
            print(f"WARNING: Large gradient norm detected: {grad_norm:.2f}, skipping update")
            self.optimizer.zero_grad()
...
def train_progress_cvae_original(
    data_path: str,
    output_dir: str = "./original_progress_cvae_results",
    batch_size: int = 128,
    epochs: int = 100,
    learning_rate: float = 1e-3,
    beta: float = 0.1,
    ...
    """Train the progress-supervised c-VAE on original expert demos."""
    ...
    for epoch in range(epochs):
        ...
        for batch_idx, (states, actions, progress) in enumerate(train_pbar):
            ...
            train_pbar.set_postfix({
                ...
                'grad': f"{loss_dict.get('grad_norm', 0.0):.2f}",
                'prog_sup': f"{loss_dict['progress_supervision_loss']:.4f}"
            })

        # Average training losses
        # Get all possible keys from all loss dicts
        all_keys = set()
        for loss in train_loss_epoch:
            all_keys.update(loss.keys())

        avg_train_loss = {}
        for key in all_keys:
            values = [loss.get(key, 0.0) for loss in train_loss_epoch]
            avg_train_loss[key] = np.mean(values)
        ...
        if avg_val_loss['total_loss'] < best_val_loss:
            ...
            model_path = output_dir / 'best_original_progress_cvae_model.pt'
            torch.save({
    ...
    curves_path = output_dir / 'original_progress_training_curves.png'
    ...
    progress_viz_path = output_dir / 'original_progress_learning_visualization.png'
    metrics = visualize_progress_learning(
        ...
        title="Progress Learning - One-Leg Assembly (Original Expert Demos)"
    )

    # Save final model
    final_model_path = output_dir / 'final_original_progress_cvae_model.pt'
    torch.save({
        'model_state_dict': model.state_dict(),
        'optimizer_state_dict': trainer.optimizer.state_dict(),
        'epoch': epochs,
...
def main():
    parser = argparse.ArgumentParser(description='Train Progress-Supervised c-VAE on Original Expert Demos')
    parser.add_argument('--data_path', type=str,
                        default='./robust-rearrangement/data/processed/diffik/sim/one_leg/teleop/low/success.zarr',
                        help='Path to original expert demo zarr dataset')
    parser.add_argument('--output_dir', type=str, default='./original_progress_cvae_results',
    ...
    # Train the model
    train_progress_cvae_original(**vars(args))
