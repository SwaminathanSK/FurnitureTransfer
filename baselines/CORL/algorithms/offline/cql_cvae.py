...
# Modified to integrate pre-trained CVAE for latent action space learning
import os
import random
import uuid
...
class HigherDimLatentCVAE(nn.Module):
    ...
    def get_latent_action(self, state: torch.Tensor, action: torch.Tensor) -> torch.Tensor:
        """Get latent action representation (using mean, no sampling)."""
        self.eval()
        with torch.no_grad():
...
class TrainConfig:
    ...
    cvae_checkpoint: Optional[str] = None  # Path to pre-trained CVAE checkpoint
...
def eval_actor(
    ...
    n_episodes: int,
    seed: int,
    cvae: Optional[nn.Module] = None,
    cql_state_mean: Union[np.ndarray, float] = 0.0,
    cql_state_std: Union[np.ndarray, float] = 1.0,
    cvae_state_mean: Optional[np.ndarray] = None,
    cvae_state_std: Optional[np.ndarray] = None,
    cvae_action_mean: Optional[np.ndarray] = None,
    cvae_action_std: Optional[np.ndarray] = None
) -> np.ndarray:
    env.seed(seed)
    actor.eval()
    episode_rewards = []
    for _ in range(n_episodes):
        ...
        episode_reward = 0.0
        while not done:
            latent_action = actor.act(state, device)
            # If CVAE is provided, decode latent action to real action
            if cvae is not None:
                # State from env is normalized by CQL's stats (via wrap_env)
                # Need to convert to CVAE's normalization for decoding
                # Step 1: Unnormalize using CQL's stats
                unnormalized_state = state * cql_state_std + cql_state_mean
                # Step 2: Normalize using CVAE's stats
                cvae_normalized_state = (unnormalized_state - cvae_state_mean) / cvae_state_std

                state_torch = torch.FloatTensor(cvae_normalized_state.reshape(1, -1)).to(device)
                latent_action_torch = torch.FloatTensor(latent_action.reshape(1, -1)).to(device)
                with torch.no_grad():
                    # CVAE decoder outputs NORMALIZED action
                    normalized_action = cvae.decode(state_torch, latent_action_torch).cpu().numpy()[0]
                    # Denormalize action to environment scale
                    action = normalized_action * cvae_action_std + cvae_action_mean
            else:
...
def train(config: TrainConfig):
    ...
    # Load CVAE and encode actions to latent space BEFORE CQL normalizes states
    # IMPORTANT: CVAE was trained with its OWN normalization - use CVAE's stats!
    cvae = None
    cvae_state_mean = None
    cvae_state_std = None
    cvae_action_mean = None
    cvae_action_std = None
    latent_action_dim = action_dim
    if config.cvae_checkpoint is not None:
        print(f"\n{'='*60}")
        print("Loading pre-trained CVAE for latent action space")
        print(f"{'='*60}")
        cvae, cvae_checkpoint = load_cvae_checkpoint(config.cvae_checkpoint, config.device)
        latent_action_dim = cvae.latent_dim

        # Get CVAE's normalization stats (CVAE was trained with normalized data)
        cvae_state_mean = cvae_checkpoint['state_mean'].cpu().numpy()
        cvae_state_std = cvae_checkpoint['state_std'].cpu().numpy()
        cvae_action_mean = cvae_checkpoint['action_mean'].cpu().numpy()
        cvae_action_std = cvae_checkpoint['action_std'].cpu().numpy()

        print(f"Using CVAE's normalization stats:")
        print(f"  CVAE state mean shape: {cvae_state_mean.shape}")
        print(f"  CVAE action mean shape: {cvae_action_mean.shape}")

        # Normalize states and actions using CVAE's normalization
        states_normalized = (dataset["observations"] - cvae_state_mean) / cvae_state_std
        actions_normalized = (dataset["actions"] - cvae_action_mean) / cvae_action_std

        # Pre-encode all dataset actions to latent space using CVAE-normalized data
        print("Pre-encoding dataset actions to latent space...")
        states_torch = torch.FloatTensor(states_normalized).to(config.device)
        actions_torch = torch.FloatTensor(actions_normalized).to(config.device)

        # Encode in batches to avoid OOM
        batch_size = 10000
        latent_actions_list = []
        for i in range(0, len(states_torch), batch_size):
            end_idx = min(i + batch_size, len(states_torch))
            batch_states = states_torch[i:end_idx]
            batch_actions = actions_torch[i:end_idx]
            latent_actions_batch = cvae.get_latent_action(batch_states, batch_actions)
            latent_actions_list.append(latent_actions_batch.cpu().numpy())

        dataset["actions"] = np.concatenate(latent_actions_list, axis=0)
        print(f"✓ Encoded {len(dataset['actions'])} actions from dim {action_dim} to latent dim {latent_action_dim}")
        print(f"{'='*60}\n")

    # Now normalize states for CQL (after encoding actions)
    if config.normalize:
    ...
    replay_buffer = ReplayBuffer(
        state_dim,
        latent_action_dim,  # Use latent action dim instead of action dim
        config.buffer_size,
        config.device,
    )
    ...
    critic_1 = FullyConnectedQFunction(
        state_dim,
        latent_action_dim,  # Use latent action dim
        config.orthogonal_init,
        config.q_n_hidden_layers,
    ).to(config.device)
    critic_2 = FullyConnectedQFunction(state_dim, latent_action_dim, config.orthogonal_init).to(
        config.device
    )
    ...
    for t in range(int(config.max_timesteps)):
        ...
        if (t + 1) % config.eval_freq == 0:
            ...
            eval_scores = eval_actor(
                ...
                seed=config.seed,
                cvae=cvae,  # Pass CVAE for decoding latent actions
                cql_state_mean=state_mean,  # CQL's normalization
                cql_state_std=state_std,
                cvae_state_mean=cvae_state_mean,  # CVAE's normalization
                cvae_state_std=cvae_state_std,
                cvae_action_mean=cvae_action_mean,  # For denormalizing decoded actions
                cvae_action_std=cvae_action_std,
            )
